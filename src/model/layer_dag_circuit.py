...
class TruthTableEncoding(nn.Module):
    ...
    def forward(self, y):
        if self.emb_size == 0:
            return torch.zeros(y.shape[0], 0).to(y.device)
...
class BiMPNNEncoder(nn.Module):
    def __init__(self,
        ...
        self.y_emb = TruthTableEncoding(y_input_dim, y_emb_size)
...
class LayerDAGCircuit(nn.Module):
    """
    针对电路数据优化的 LayerDAG 模型。
    """
    def __init__(self,
                 device,
                 num_x_n_cat,
        ...
        self.y_input_dim = y_input_dim # Store this for padding logic
        ...
        node_pred_graph_encoder = BiMPNNEncoder(num_x_n_cat, 
                                                hidden_size=hidden_size,
                                                y_input_dim=y_input_dim,
        ...
        edge_pred_graph_encoder = BiMPNNEncoder(num_x_n_cat, 
                                                hidden_size=hidden_size,
                                                y_input_dim=y_input_dim,
    ...
    # ... [Omitted posterior, posterior_edge, sample_node_layer, sample_edge_layer, get_batch_A, get_batch_A_n2g 
    #      because they are identical to previous version and lengthy. 
    #      Please refer to previous context for them if needed, or I can regenerate fully.] ...
    
    # RE-INCLUDING THEM TO BE SAFE AND COMPLETE as this is the "Most up-to-date" file
    ...
    def posterior_edge(self, Z_t, alpha_t, alpha_bar_s, alpha_bar_t, Z_0,
                       marginal_list, num_new_nodes_list, num_query_list):
    ...
    def sample_node_layer(self, A, x_n, abs_level, rel_level, A_n2g,
                          curr_level=None, y=None, min_num_steps_n=None,
    ...
    def sample_edge_layer(self, num_nodes_cumsum, edge_index_list,
        ...
        for s_x_e in time_x_e_list:
            ...
            e_t_mask = self.posterior_edge(e_t_one_hot, alpha_t, alpha_bar_s,
                                           alpha_bar_t, e_0_probs, marginal_list,
                                           num_new_nodes_list, num_query_list)
    ...
    def get_batch_y(self, y_list, x_n_list, device):
        """
        处理 batch 中的 y，支持不同长度的 truth table，通过 padding 到 self.y_input_dim。
        """
        if y_list is None:
            return None
        ...
        for i in range(len(x_n_list)):
            ...
            # 1. Flatten to 1D
            y_i = y_i.reshape(-1) # (y_len,)
            
            # 2. Pad or truncate to match self.y_input_dim
            current_len = y_i.shape[0]
            if current_len < self.y_input_dim:
                # Pad with zeros
                pad_size = self.y_input_dim - current_len
                y_i = F.pad(y_i, (0, pad_size), 'constant', 0)
            elif current_len > self.y_input_dim:
                # Truncate (warning: info loss)
                y_i = y_i[:self.y_input_dim]
                
            # 3. Add dimension for concatenation (1, y_input_dim)
            y_i = y_i.unsqueeze(0)
            
            # 4. Expand to num nodes
            y_list_.append(y_i.expand(len(x_n_list[i]), -1))
